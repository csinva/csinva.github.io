...
* only kernel can access resources provided by hardware
 ...
 1. system programs - associated with OS but not in kernel
...
* memory
  * *von Neumann architecture* - uses instruction register
  * main memory is *RAM*
...
* more common
  ...
  1. *blade server* - multiple independence multiprocessor systems in same chassis
...
* kernel can use a timer to avoid getting stuck in user mode
...
* maintain list of *user IDs* and *group IDs*
  * can temporarily *escalate privileges* to an *effective UID* - *setuid* command
...
* out points to first full position
 ...
 1. *message passing* - messages between coordinating processes
...
* uses #pragma
 ...
 1. *Grand central dispatch* - extends C
...
* 3 requirements
 ...
 2. *progress* - things should be in critical section
 3. *bounded waiting* - every process should eventually get to critical section
...
* *monitor* - highl-level synchronization construct
  ...
  * *abstract data type* which includes a set of programmer-defined operations with mutual exclusion
...
2. *dispatch latency*
3. Preemption of any process running in the kernel
4. Release by low-priority processes of resources needed by a high-priority process
...
* *demand paging* - load pages only when they are needed
  ...
  * can use valid-invalid bit in page table to signal whether a page is in memory
...
* replace the page that will not be used for the longest period of time
 ...
 1. LRU - least recently used (last used)
 1. implement with counters since each use
 1. stack of page numbers (whenever something is used, put it on top)
...
* give preference to pages that have been modified
 ...
 1. counting-based - count and implement LFU (least frequently used) or MFU (most frequently used)

* page-buffering algorithms
  * pool of free frames - makes things faster
  * list of modified pages - written to disk whenever paging device is idle
  * some algorithms, like databases, perform better when they get their own memory capability called *raw disk* instead of being managed by OS

## 8.5 *frame-allocation algorithms* - how many frames to allocate to each process in memory (skipped)
