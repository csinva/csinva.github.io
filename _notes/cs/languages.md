...
```python
l = ['abc', 'ccc', 'd', 'bb']
- sorted(l, reverse=False, key=len) ## ascending order (set reverse=True for decreasing)
...
* dask
  ...
  * configuration: specify sequential ('synchronous'), threads within current session ('threaded') parallel ('processes') multiprocessing - this creates copies of objects, multi-node or not ('distributed')
  * need to wrap functions with `delayed()` or `map()`
    * alternatively can tag functions
  * map() operation does caching
  * dask dataframes
  * ssh setup
    * can connect to other workers locally (ssh-dask or SSHCluster)
...
Pointers
Stores a memory address of another object //we will assume everything is 32 bit
...
3. Has implicit dereferencing
   ...
   C++ default class
4. Destructor                                           //this will do nothing
   Frees up any resources allocated during the use of an object
5. Copy Constructor                                     //copies something over
